...
function renderEmployees() {
    ...
    tbody.innerHTML = `<tr><td colspan="${rosterVisibleColumnCount()}">
      <div class="empty-entry-wrap">
        ...
        <div class="empty-entry-cards">
          <button class="quick-entry-card quick-entry--single" type="button" id="emptyAddEmployee">
          ...
          <button class="quick-entry-card quick-entry--batch" type="button" id="emptyBatchEmployee">
            <span class="quick-entry-icon" aria-hidden="true">
  ...
  tbody.innerHTML = Array.from(groups.values()).map(group => {
    ...
    const pending = group.rows.filter(row => Number(row.employeeStatus) === 1).length;
    const offboarding = group.rows.filter(row => row.lifecycleStatus === 'OFFBOARDING').length;
    ...
    const head = `<tr class="customer-group-row"><td colspan="${rosterVisibleColumnCount()}"><div><strong>${escapeHtml(group.name)}</strong><span>${group.rows.length} 人</span><span>在职 ${active}</span><span>待入职 ${pending}</span><span class="${offboarding ? 'risk-text' : ''}">离职中 ${offboarding}</span><span class="${insuranceGap ? 'risk-text' : ''}">雇主险待增 ${insuranceGap}</span><span class="${risk ? 'risk-text' : ''}">风险 ${risk}</span></div></td></tr>`;
...
function renderCustomerRosterRail() {
  ...
  rail.innerHTML = Array.from(groups.values()).map(group => {
    ...
    return `<button class="customer-roster-card" type="button" data-customer-roster="${group.id}"><span>${escapeHtml(group.name)}</span><strong>${group.rows.length}<small>人</small></strong><div><em>在职 ${active}</em><em>待入职 ${pending}</em><em class="${offboarding ? 'risk' : ''}">离职中 ${offboarding}</em><em class="${insuranceGap ? 'risk' : ''}">雇主险 ${insuranceGap}</em><em class="${risks ? 'risk' : ''}">风险 ${risks}</em></div></button>`;
