...
async function main() {
  ...
  await withDbStubs({
    ...
    assert.equal(result.rows[0].grossAmount, 0, '不得根据基本工资自动计算应发工资');
    assert.equal(result.rows[0].netAmount, 0, '不得根据应发和扣款自动计算实发工资');
    assert.equal(result.rows[1].employeeId, 89);
    assert.equal(result.rows[1].baseSalary, 0, '应发工资不得自动补写基本工资');
    ...
    assert.equal(result.rows[3].grossAmount, 0, '未上传应发工资时不得自动计算');
    assert.equal(result.rows[3].netAmount, 0, '未上传实发工资时不得自动计算');
  });
  ...
  await withDbStubs({
    ...
    assert.equal(result.rows[0].otherDeduction, 0, '扣款类目仅按原表展示，不进入系统计算字段');
  });
