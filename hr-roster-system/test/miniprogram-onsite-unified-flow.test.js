...
for (const stage of ['', 'pending', 'active', 'left']) {
  const marker = stage ? `data-stage="${stage}"` : 'data-stage=""';
  if (!onsiteWxml.includes(marker)) throw new Error(`蓝色驻厂面板缺少人员状态入口：${stage || '全部'}`);
}
if ((onsiteWxml.match(/bindtap="filterByStage"/g) || []).length !== 4) throw new Error('蓝色驻厂面板应只保留四个可点击状态');
...
if (/已减保|雇主险减保/.test(resignWxml)) throw new Error('驻厂快速离职仍要求雇主险减保');
...
if (/INSURANCE_TERMINATION|goOnsiteTask|\/work-tasks/.test(homeJs)) throw new Error('工作台仍保留驻厂处理队列');
