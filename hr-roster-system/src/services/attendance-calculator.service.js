...
function calculateDailyAttendance({ schedule, punches = [] }) {
  ...
  if (!first || !last || last <= first) return { firstInAt: iso(first), lastOutAt: iso(last), workedMinutes: 0, approvedNormalMinutes: 0, overtimeCandidateMinutes: 0, lateMinutes: 0, earlyLeaveMinutes: 0, resultStatus: 'MISSING_PUNCH' };
  ...
  return { firstInAt: iso(first), lastOutAt: iso(last), workedMinutes: worked, approvedNormalMinutes: Math.min(worked, Number(schedule.standardMinutes)), overtimeCandidateMinutes: overtime, lateMinutes: late, earlyLeaveMinutes: early, resultStatus: late ? 'LATE' : early ? 'EARLY_LEAVE' : 'NORMAL' };
}
