...
function normalizeFeeMode(value) {
  const raw = String(value || '').trim();
  if (!raw) throw createError('费用模式不能为空，请自行填写');
...
async function getBootstrap(companyId, user = null) {
  ...
  const [company, allDepartments, positions, allCustomers, allProjects, recruiters, suppliers] = await Promise.all([
    ...
    db.query(
      'SELECT id, company_id AS companyId, position_name AS positionName, position_code AS positionCode, risk_level AS riskLevel, is_special_work AS isSpecialWork, status FROM hr_position WHERE company_id = :companyId AND status = 1 ORDER BY id',
      { companyId }
    ),
    db.query(
...
async function getSummary(companyId, user = null) {
  ...
  const risk = await db.first(
    ...
    LEFT JOIN hr_employee_job j ON j.employee_id=e.id AND j.company_id=e.company_id AND j.job_status=1
    WHERE r.company_id = :companyId ${employeeFilter}
    `,
    params
  );
...
async function getEmployeeDetail(companyId, employeeId, options = {}) {
  ...
  return {
    ...
    resignation: resignation ? {
      ...
      settlementStatus: Number(resignation.settlement_status || 0),
...
async function validateEmployeeInput(companyId, body, employeeId = 0, connection = db.pool) {
  const required = [
    ['name', '姓名不能为空'],
    ['phone', '手机号不能为空'],
    ['idCardNo', '身份证号不能为空'],
  ...
  if (![1, 2, 3, 4, 5, 6].includes(Number(body.employmentType))) throw createError('用工模式不正确');
  normalizeFeeMode(body.feeMode);
  if (![1, 2, 3].includes(Number(body.workType))) throw createError('工资类型不正确');
...
async function validateRecruitmentSource(companyId, body, connection = db.pool) {
  ...
  if (![1, 2].includes(sourceType)) throw createError('请填写招聘渠道');
...
async function syncResignationCompletion(connection, companyId, resignationId, operatorId = 0) {
  ...
  const handoverDone = ['badge_returned', 'tools_returned', 'dorm_cleared', 'attendance_confirmed']
    .every(field => Number(row[field]) === 1);
  // 合规模块仅保留雇主险增减；历史社保状态不得继续阻塞员工离职闭环。
  const insuranceDone = Number(row.employer_insurance_status || 0) !== 1;
  const settlementDone = Number(row.settlement_status) === 1;
  ...
  for (const [taskType, done] of [
    ...
    ['PAYROLL_SETTLEMENT', settlementDone]
  ]) {
    if (done) {
      await connection.execute(
  ...
  const completed = handoverDone && insuranceDone && settlementDone;
  if (completed) {
    await connection.execute(
  ...
  return { employeeId: row.employee_id, resignationId, handoverDone, insuranceDone, settlementDone, completed };
}
...
async function createEmployee(companyId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    await validateEmployeeInput(companyId, normalizedBody, 0, connection);
    // 所有客户端缺省均进入待入职，避免某一端漏传状态后直接成为在职员工。
    const employeeStatus = [1, 2].includes(Number(body.employeeStatus)) ? Number(body.employeeStatus) : 1;

    const [employeeResult] = await connection.execute(
      `
      INSERT INTO hr_employee
      (company_id, employee_no, name, gender, id_card_no, id_card_hash, phone, email, education, bank_name, bank_card_no,
      ...
      VALUES
      (:companyId, :employeeNo, :name, :gender, :idCardNo, :idCardHash, :phone, :email, :education, :bankName, :bankCardNo,
       :emergencyContact, :emergencyPhone, :channelSource, :recruitmentChannelId, :recruitmentSourceType, :recruiterId, :supplierId,
       :lifecycleStatus, 'PENDING', :remark, :employeeStatus, :createdBy)
      `,
      {
        companyId,
        ...
        lifecycleStatus: employeeStatus === 1 ? 'PENDING_ARRIVAL' : 'ACTIVE',
...
async function createEmployeesBatch(companyId, rows, operatorId = 0, user = null) {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    try {
      ...
      if (!customerId) throw createError(`客户单位"${row.customerName || ''}"不存在`);
      if (!positionId) throw createError(`岗位"${row.positionName || ''}"不存在`);
      const projectName = String(row.projectName || '').trim();
      const projectId = projectName ? projectMap.get(`${customerId}::${projectName}`) : null;
      if (projectName && !projectId) throw createError(`所属项目"${projectName}"不存在或不属于所选客户`);

      // 用工模式和费用模式必须由录入人员明确填写。
...
async function updateEmployee(companyId, employeeId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    await validateEmployeeInput(companyId, normalizedBody, employeeId, connection);
...
async function transferJob(companyId, employeeId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    const targetProjectId = Number(body.newProjectId || 0) || null;
    ...
    const crossProject = Boolean(targetProjectId && currentProjectId !== targetProjectId);
...
async function resignEmployee(companyId, employeeId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    if (employee.lifecycle_status === 'OFFBOARDING') throw createError('员工正在办理离职，请在待办中心更新进度');
    if (!body.leaveDate || !body.leaveType || !body.leaveReason) throw createError('请完整填写离职信息');
    const canManageSettlement = Array.isArray(user?.permissions) && user.permissions.includes('payroll:manage');
    if (Number(body.settlementStatus || 0) === 1 && !canManageSettlement) {
      throw createError('工资结算状态只能由薪资专员或企业管理员确认', 403);
    }
    ...
    const [resignationResult] = await connection.execute(
      ...
      `,
      {
        ...
        handoverStatus: 0,
        badgeReturned: Number(body.badgeReturned || 0) === 1 ? 1 : 0,
        toolsReturned: Number(body.toolsReturned || 0) === 1 ? 1 : 0,
        dormCleared: Number(body.dormCleared || 0) === 1 ? 1 : 0,
        attendanceConfirmed: Number(body.attendanceConfirmed || 0) === 1 ? 1 : 0,
        settlementStatus: canManageSettlement && Number(body.settlementStatus || 0) === 1 ? 1 : 0,
        riskRemark: body.riskRemark || null
      }
    );

    await connection.execute(
      "UPDATE hr_employee SET lifecycle_status='OFFBOARDING',updated_at=NOW() WHERE company_id=:companyId AND id=:employeeId",
      { companyId, employeeId }
    );
    ...
    await connection.execute(
      ...
      VALUES
      (:companyId, :employeeId, 6, 2, '离职雇主险减保提醒', :riskDesc, :riskKey, 0)
      ON DUPLICATE KEY UPDATE updated_at = updated_at
      `,
      {
        companyId,
        employeeId,
        riskDesc: `${employee.name}已办理离职，请确认雇主险减保和工资结算`,
        riskKey: `resign_followup:${resignationResult.insertId}`
      }
    );
    ...
    await noticeService.createNotice(connection, {
      companyId,
      employeeId,
      title: `${employee.name}已发起离职，请完成交接、雇主险减保和工资结算`,
      category: '离职待办',
      noticeType: 'warning',
      targetView: 'roster',
    ...
    for (const task of [
      { taskType: 'OFFBOARD', taskTitle: `${employee.name}离职交接待确认`, riskLevel: 2, taskContent: '请确认工牌、工服工具、宿舍和考勤四项交接' },
      { taskType: 'INSURANCE_TERMINATION', taskTitle: `${employee.name}离职待减雇主险`, riskLevel: 3 },
      { taskType: 'PAYROLL_SETTLEMENT', taskTitle: `${employee.name}离职工资待结算`, riskLevel: 2 }
    ]) {
      await createWorkTask(connection, {
        companyId,
        employeeId,
        projectId: null,
        ...task,
        taskContent: task.taskContent || '请完成离职后续事项并上传处理凭证',
        sourceType: 'RESIGNATION',
        sourceId: resignationResult.insertId,
        assignedUserId: operatorId || null,
        deadline: `${body.leaveDate} 23:59:59`
      });
    }
...
async function updateResignationProgress(companyId, resignationId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    const canManageHandover = permissions.includes('employee:resign');
    const canManageSettlement = permissions.includes('payroll:manage');
    const handoverFields = ['badgeReturned', 'toolsReturned', 'dormCleared', 'attendanceConfirmed', 'riskRemark'];
    if (!canManageHandover && handoverFields.some(field => body[field] !== undefined)) {
      throw createError('当前账号只能处理工资结算，不能修改现场交接', 403);
    }
    if (!canManageSettlement && body.settlementStatus !== undefined) {
      throw createError('工资结算状态只能由薪资专员或企业管理员确认', 403);
    }
    if (!canManageHandover && !canManageSettlement) throw createError('无离职进度处理权限', 403);
    ...
    const payload = {
      ...
      settlementStatus: value('settlementStatus', resignation.settlement_status),
    ...
    await connection.execute(
      `UPDATE hr_resignation
       SET badge_returned=:badgeReturned,tools_returned=:toolsReturned,dorm_cleared=:dormCleared,
           attendance_confirmed=:attendanceConfirmed,settlement_status=:settlementStatus,
...
async function onboardEmployee(companyId, employeeId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    await assertEmployeeScope(companyId, employeeId, user, connection);
    const [[employee]] = await connection.execute(
      'SELECT id, name, employee_status FROM hr_employee WHERE company_id=:companyId AND id=:employeeId AND deleted_at IS NULL LIMIT 1',
      { companyId, employeeId }
    );
    if (!employee) throw createError('员工不存在', 404);
    ...
    if (!body.hireDate) throw createError('入职日期不能为空');

    const [[currentJob]] = await connection.execute(
      'SELECT id FROM hr_employee_job WHERE company_id=:companyId AND employee_id=:employeeId AND job_status=1 LIMIT 1',
      { companyId, employeeId }
    );
    if (!currentJob) throw createError('员工任职信息不存在');
    ...
    await connection.execute(
      ...
      { hireDate: body.hireDate, remark: body.remark || null, jobId: currentJob.id }
    );
    ...
    const reminders = [
      {
        riskType: 1,
        riskTitle: '入职合同签署提醒',
        riskDesc: `${employee.name}已确认入职，请及时完成劳动合同签署`,
        riskKey: `onboard_contract:${employeeId}`
      },
      {
        riskType: 3,
        riskTitle: '入职保险办理提醒',
        riskDesc: `${employee.name}已确认入职，请办理雇主险增保`,
        riskKey: `onboard_insurance:${employeeId}`
      }
    ];
    for (const reminder of reminders) {
      await connection.execute(
        `INSERT INTO hr_risk_alert
         (company_id, employee_id, risk_type, risk_level, risk_title, risk_desc, risk_key, handle_status)
         VALUES (:companyId, :employeeId, :riskType, 2, :riskTitle, :riskDesc, :riskKey, 0)
         ON DUPLICATE KEY UPDATE risk_desc=VALUES(risk_desc), updated_at=NOW()`,
      ...
      await noticeService.createNotice(connection, {
        ...
        category: '入职待办',
        noticeType: 'warning',
        targetView: reminder.riskType === 1 ? 'roster' : 'insurance',
    ...
    await createWorkTask(connection, {
      ...
      projectId: null,
      taskType: 'INSURANCE',
      taskTitle: `${employee.name}到岗待投保`,
      taskContent: '员工已确认到岗，请办理雇主险增保',
      sourceType: 'EMPLOYEE_ONBOARDING',
      sourceId: employeeId,
      riskLevel: 3,
      assignedUserId: operatorId || null,
      deadline: `${body.hireDate} 23:59:59`
    });
    ...
    await connection.execute(
       ...
       VALUES (:companyId, :operatorId, '员工入职', 'employee', :employeeId, 'onboard', :beforeData, :afterData)`,
      {
        ...
        employeeId,
        beforeData: JSON.stringify({ employeeStatus: employee.employee_status }),
        afterData: JSON.stringify({ employeeStatus: 2, hireDate: body.hireDate, remark: body.remark || '' })
      }
    );

    return { employeeId, employeeStatus: 2, hireDate: body.hireDate };
  });
}
...
async function createContract(companyId, employeeId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    if (!body.contractNo) throw createError('合同编号不能为空');
    if (!body.contractType) throw createError('合同类型不能为空');
    if (!body.startDate) throw createError('合同开始日期不能为空');
    if (body.endDate && body.endDate < body.startDate) throw createError('合同结束日期不能早于开始日期');
    ...
    await connection.execute(
      ...
      `,
      {
        ...
        afterData: JSON.stringify({ employeeId, contractNo: body.contractNo })
      }
    );
    ...
    await noticeService.createNotice(connection, {
      ...
      category: '合同变更',
      noticeType: Number(body.signStatus || 0) === 1 ? 'success' : 'warning',
      targetView: 'roster',
      dedupeKey: `contract:${result.insertId}`
    });

    if (Number(body.signStatus || 0) === 1) {
      await connection.execute(
...
async function updateSocialSecurity(companyId, employeeId, body, operatorId = 0, user = null) {
  return db.transaction(async connection => {
    ...
    if (employerInsuranceAction === 'ADD') {
      ...
      await connection.execute(
        `UPDATE hr_risk_alert SET handle_status=2,handled_by=:operatorId,handled_at=NOW(),handle_note='雇主险已增保'
